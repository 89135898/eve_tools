...
当前切片由 worker 使用公开 ESI 同步市场订单到 Supabase；桌面端通过 hosted HTTP API 读取最新成功快照做市场查询和选品发现。fixture 数据仅用于显式开发模式，不在生产 live 模式下静默回退。静态 SDE catalog 数据通过 Rust catalog service 导入 Supabase Postgres。EVE SSO 和认证角色订单同步会在后续实现阶段处理。
...
桌面端只读取 `EVETOOLS_API_BASE_URL`，不再直接读取 `EVETOOLS_DATABASE_URL`。运行时环境变量优先生效；没有运行时变量时，Tauri 使用 `apps/desktop/.env.production` 在构建时注入的默认地址。`EVETOOLS_DATABASE_URL` 只应存在于服务端 HTTP API、worker CLI、catalog admin CLI 或托管 job 环境中。
