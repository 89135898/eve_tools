...
- 基于 hosted API 和 Supabase 快照的市场价格查询
- 基于 hosted API 和 Supabase 快照的选品发现
- EVE SSO 登录、认证角色订单同步和订单监控建议

当前切片由 worker 使用公开 ESI 同步市场订单到 Supabase；桌面端通过 hosted HTTP API 读取最新成功快照做市场查询和选品发现。角色订单监控通过 EVE SSO + PKCE 授权读取角色公开市场订单，并与公开盘口快照做只读改价建议。fixture 数据仅用于显式开发模式，不在生产 live 模式下静默回退。静态 SDE catalog 数据通过 Rust catalog service 导入 Supabase Postgres。
...
桌面客户端的默认 hosted API 地址配置在 `apps/desktop/.env.production`。当前默认值为：

```env
EVETOOLS_API_BASE_URL=http://13.158.109.65:8080
```

`pnpm build` 会自动读取该文件并把地址注入 Tauri/Rust 构建环境。运行时 shell 中的 `EVETOOLS_API_BASE_URL` 仍然优先生效，便于临时测试其他后端；本地开发也可以创建不提交的 `apps/desktop/.env.local` 覆盖默认地址。

本地或私有桌面 SSO 订单同步还需要在运行 Tauri 的 Rust 进程中配置：

```env
EVETOOLS_DATABASE_URL=<postgres-url-with-sslmode-require>
EVETOOLS_SSO_CLIENT_ID=<eve-developer-application-client-id>
EVETOOLS_SSO_REDIRECT_URI=http://127.0.0.1:17813/callback
```

`EVETOOLS_SSO_BASE_URL` 默认是 `https://login.eveonline.com`；`EVETOOLS_ESI_BASE_URL` 仅用于 mock/测试。不要把真实数据库 URL、refresh token、access token 或 EVE client id 写入已提交文件；用本地 shell 环境或未提交的 `.env.local`。
...
未设置该变量时，桌面端默认使用 `live`。`fixture` 只用于本地开发或演示。
...
- hosted API 不可用、本地未配置 API base URL 或未同步快照时，live 模式会显示错误和后端健康检查状态，不会静默使用 fixture 数据。
...
桌面端已接入 EVE SSO Authorization Code + PKCE。EVE Developer 应用的 callback URL 必须与 `EVETOOLS_SSO_REDIRECT_URI` 完全一致，推荐本地 loopback，例如 `http://127.0.0.1:17813/callback`。需要授权 scope：
...
1. React 调用 Tauri command `start_eve_sso_login`。
2. Rust 生成 PKCE verifier/challenge 和 state，打开系统浏览器。
3. 本地 loopback listener 接收 EVE SSO callback，交换 token。
4. Rust 解码 token 中的角色身份并校验 scope。
5. `character_auth_tokens` 保存 refresh token 和短期 access token；React/TypeScript 不接触 token。

同步流程：

1. React 调用 `sync_character_orders`。
2. Rust/worker 按需刷新 access token。
3. ESI `GET /latest/characters/{character_id}/orders/` 拉取角色订单。
4. DB 写入 `character_order_sync_runs` 和 `character_order_snapshots`。
5. Order Monitor 读取 `/characters/{character_id}/order-monitor?language=...`，将角色订单与公开 station order book 比较，输出 `raise`、`lower` 或 `hold` 建议。

该功能只做 advisory repricing，不会自动下单、改单或撤单。错误信息会裁剪并脱敏，避免把 token 写入同步失败记录。当前本地/私有桌面模式下，Tauri Rust 进程需要 `EVETOOLS_DATABASE_URL` 来保存 token 和快照；不要把该连接串暴露给 React、WebView storage、日志、截图或面向不受信任用户的 release 包。
...
公开市场生产同步的数据库连接串只应配置在服务端 HTTP API、worker、catalog admin CLI 或托管 job 环境中。认证订单同步的本地/私有桌面模式会在 Tauri Rust 进程中读取 `EVETOOLS_DATABASE_URL`；不要把它放进 React 代码、WebView storage 或面向不受信任用户的打包配置。
...
- `GET /characters/{character_id}/order-monitor?language=zh-CN&limit=500`

桌面端公开读路径使用 `EVETOOLS_API_BASE_URL`。SSO 登录和角色订单同步路径额外使用 `EVETOOLS_DATABASE_URL`、`EVETOOLS_SSO_CLIENT_ID` 和 `EVETOOLS_SSO_REDIRECT_URI`，这些值只在 Rust/Tauri 层使用，不会作为 React state 或 TypeScript token 类型暴露。
...
- `0005_add_authenticated_order_monitor.sql`：EVE SSO 角色、auth token、角色订单同步运行和订单快照表。
...
Direct Supabase Postgres 模式适用于本地、私有桌面、admin catalog 导入、worker 同步和 hosted HTTP API 服务端环境。`EVETOOLS_DATABASE_URL` 是特权凭据：不要把它暴露给 React，不要提交到仓库，不要注入给不受信任的最终用户。若要发布给多用户，应把 SSO/token 存储和同步移到受控后端，而不是把数据库连接串打包进桌面应用。
...
桌面端查询 catalog、Market Price Lookup、trade hubs 和 Selection Discovery 时通过 `EVETOOLS_API_BASE_URL` 调用 hosted HTTP API。EVE SSO 和角色订单同步由 Tauri Rust 命令处理 token、数据库写入和 worker 调用；React 只接收 auth status、sync summary 和 order monitor rows。
...
- `Order Monitor`：登录 EVE 后同步角色订单，展示真实角色订单的建议动作和紧急度；显式 fixture 模式仍可用于本地演示。

同步状态分为公开和私有流程：

- Public market sync：`live-ready` 或 `fixture-ready`
- Authenticated order sync：`not-configured`、`not-authorized`、`authorized`、`success` 或 `failed`
